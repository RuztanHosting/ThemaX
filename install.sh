...
installTheme(){
    ...
    tar -cvf ThemaXbackup.tar.gz pterodactyl > /dev/null 2>&1
    echo -e "${GREEN}Installing theme... ${RESET}"
    cd /var/www/pterodactyl > /dev/null 2>&1
    echo -e "${GREEN}Removing old theme if exist${RESET}"
    rm -r ThemaX > /dev/null 2>&1
    echo -e "${GREEN}Download the Theme${RESET}"
    git clone https://github.com/RuztanHosting/ThemaX.git > /dev/null 2>&1
    cd ThemaX > /dev/null 2>&1
    echo -e "${GREEN}Removing old theme resources if exist${RESET}"
    rm /var/www/pterodactyl/resources/scripts/ThemaX.css > /dev/null 2>&1
    ...
    mv ThemaX.css /var/www/pterodactyl/resources/scripts/ThemaX.css > /dev/null 2>&1
    cd /var/www/pterodactyl > /dev/null 2>&1
...
repair(){
    bash <(curl https://raw.githubusercontent.com/RuztanHosting/ThemaX/main/repair.sh)
}

restoreBackUp(){
    echo "Restoring backup..."
    cd /var/www/ > /dev/null 2>&1
    tar -xvf ThemaXbackup.tar.gz > /dev/null 2>&1
    rm ThemaXbackup.tar.gz > /dev/null 2>&1
